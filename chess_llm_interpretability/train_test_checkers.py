...
# meta is used to encode the string pdn strings into integer sequences
...
def get_board_seqs_int(df: pd.DataFrame) -> Int[Tensor, "num_games pdn_str_length"]:
...
def get_custom_indices(
    ...
    custom_indices = checkers_utils.find_custom_indices(custom_indexing_function, df)
...
def prepare_data_batch(
    ...
    games_int = probe_data.board_seqs_int[indices]  # games_int shape (batch_size, pdn_str_length)
    ...
    state_stack = checkers_utils.create_state_stacks(
        games_str, config.custom_board_state_function, games_skill
    )  # shape (modes, batch_size, pdn_str_length, num_rows, num_cols)
    ...
    state_stack_one_hot = checkers_utils.state_stack_to_one_hot(
        TRAIN_PARAMS.modes,
        config.num_rows,
        config.num_cols,
        config.min_val,
    ...
    with torch.inference_mode():
        ...
        for layer in layers:
            resid_post_dict[layer] = cache["resid_post", layer][
                :, :
            ]  # shape (batch_size, pdn_str_length - 1, d_model)
    ...
    if args.mode == "test":
        ...
        for probe_to_test in saved_probes:
            ...
            with open(probe_file_location, "rb") as f:
                ...
                config = checkers_utils.find_config_by_name(state_dict["config_name"])
                ...
                config = checkers_utils.set_config_min_max_vals_and_column_name(
        ...
        config = checkers_utils.piece_config
        if args.probe == "skill":
            raise "We don't do skill probing"
            config = checkers_utils.skill_config
        ...
        config = checkers_utils.update_config_using_player_color(player_color, config)
